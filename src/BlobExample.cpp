		...
		if(result->total==3 && fabs(cvContourArea(result, CV_WHOLE_SEQ))>100 )
		{
			...
			if(posX > 360 ){
				if(lastX1>=0 && lastY1>=0 && posX>=0 && posY>=0)
				{
					...
					cvLine(imgTracking, *pt[0],*pt[1], cvScalar(198,243,23),2);
					cvLine(imgTracking, *pt[1],*pt[2],cvScalar(198,243,23),2);
					cvLine(imgTracking, *pt[2], *pt[0], cvScalar(198,243,23),2);
				}
